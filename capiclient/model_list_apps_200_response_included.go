/*
Cloud Foundry API

OpenAPI specification for the Cloud Foundry API.

API version: 3.195.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package capiclient

import (
	"encoding/json"
)

// checks if the ListApps200ResponseIncluded type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ListApps200ResponseIncluded{}

// ListApps200ResponseIncluded struct for ListApps200ResponseIncluded
type ListApps200ResponseIncluded struct {
	Organizations []Organization `json:"organizations,omitempty"`
	Spaces []Space `json:"spaces,omitempty"`
}

// NewListApps200ResponseIncluded instantiates a new ListApps200ResponseIncluded object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewListApps200ResponseIncluded() *ListApps200ResponseIncluded {
	this := ListApps200ResponseIncluded{}
	return &this
}

// NewListApps200ResponseIncludedWithDefaults instantiates a new ListApps200ResponseIncluded object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewListApps200ResponseIncludedWithDefaults() *ListApps200ResponseIncluded {
	this := ListApps200ResponseIncluded{}
	return &this
}

// GetOrganizations returns the Organizations field value if set, zero value otherwise.
func (o *ListApps200ResponseIncluded) GetOrganizations() []Organization {
	if o == nil || IsNil(o.Organizations) {
		var ret []Organization
		return ret
	}
	return o.Organizations
}

// GetOrganizationsOk returns a tuple with the Organizations field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ListApps200ResponseIncluded) GetOrganizationsOk() ([]Organization, bool) {
	if o == nil || IsNil(o.Organizations) {
		return nil, false
	}
	return o.Organizations, true
}

// HasOrganizations returns a boolean if a field has been set.
func (o *ListApps200ResponseIncluded) HasOrganizations() bool {
	if o != nil && !IsNil(o.Organizations) {
		return true
	}

	return false
}

// SetOrganizations gets a reference to the given []Organization and assigns it to the Organizations field.
func (o *ListApps200ResponseIncluded) SetOrganizations(v []Organization) {
	o.Organizations = v
}

// GetSpaces returns the Spaces field value if set, zero value otherwise.
func (o *ListApps200ResponseIncluded) GetSpaces() []Space {
	if o == nil || IsNil(o.Spaces) {
		var ret []Space
		return ret
	}
	return o.Spaces
}

// GetSpacesOk returns a tuple with the Spaces field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ListApps200ResponseIncluded) GetSpacesOk() ([]Space, bool) {
	if o == nil || IsNil(o.Spaces) {
		return nil, false
	}
	return o.Spaces, true
}

// HasSpaces returns a boolean if a field has been set.
func (o *ListApps200ResponseIncluded) HasSpaces() bool {
	if o != nil && !IsNil(o.Spaces) {
		return true
	}

	return false
}

// SetSpaces gets a reference to the given []Space and assigns it to the Spaces field.
func (o *ListApps200ResponseIncluded) SetSpaces(v []Space) {
	o.Spaces = v
}

func (o ListApps200ResponseIncluded) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ListApps200ResponseIncluded) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Organizations) {
		toSerialize["organizations"] = o.Organizations
	}
	if !IsNil(o.Spaces) {
		toSerialize["spaces"] = o.Spaces
	}
	return toSerialize, nil
}

type NullableListApps200ResponseIncluded struct {
	value *ListApps200ResponseIncluded
	isSet bool
}

func (v NullableListApps200ResponseIncluded) Get() *ListApps200ResponseIncluded {
	return v.value
}

func (v *NullableListApps200ResponseIncluded) Set(val *ListApps200ResponseIncluded) {
	v.value = val
	v.isSet = true
}

func (v NullableListApps200ResponseIncluded) IsSet() bool {
	return v.isSet
}

func (v *NullableListApps200ResponseIncluded) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableListApps200ResponseIncluded(val *ListApps200ResponseIncluded) *NullableListApps200ResponseIncluded {
	return &NullableListApps200ResponseIncluded{value: val, isSet: true}
}

func (v NullableListApps200ResponseIncluded) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableListApps200ResponseIncluded) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


